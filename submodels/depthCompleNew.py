...
class depthCompletionNew2(nn.Module):
    def __init__(self,bs):
        super(depthCompletionNew2, self).__init__()
        self.bs = bs
...
class maskFt2(nn.Module):
    def __init__(self):
        super(maskFt2, self).__init__()
        self.maskC3_block = maskFt()
        self.maskN3_block = maskFt()

    def forward(self,tempCatC,tempCatN):
        maskC3 = self.maskC3_block(tempCatC)
        maskN3 = self.maskN3_block(tempCatN)
        return maskC3,maskN3

class depthCompletionNew(nn.Module):
    def __init__(self, bs):
        super(depthCompletionNew, self).__init__()
        self.bs = bs
        self.normal = depthCompletionNew2(bs)
        self.outC_block = depthCompletionNew_block(bs)
        self.outN_block = depthCompletionNew_blockN(bs)
        self.maskFt_block = maskFt2()

    def forward(self, left,sparse,mask):
        normal_in = self.normal(left, sparse, mask)

        outC,maskC,tempCatC = self.outC_block(left, sparse, mask)

        outN,tempCatN = self.outN_block(normal_in, sparse, maskC)

        maskC3,maskN3 = self.maskFt_block(tempCatC,tempCatN)

        if self.training:
            return outC,outN,maskC3,maskN3,normal_in
        else:
            return outC,outN,maskC3,maskN3
